...
```mermaid
graph TD
    User["User (Browser/iPhone)"] -->|"HTTPS (443)"| Nginx["Nginx (Host)"]
    Nginx -->|"Proxy Pass (8000)"| Docker["Docker Container"]
    
    subgraph "Docker Container (App)"
        FastAPI["FastAPI Backend"]
        Static["React Static Files"]
        FastAPI -->|Serves| Static
        FastAPI -->|Async Calls| SchwabAPI[Charles Schwab API]
        FastAPI -->|Read/Write| SQLite[(SQLite DB)]
    end
```
